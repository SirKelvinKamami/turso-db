...
```powershell
# Login and get token
$token = (Invoke-RestMethod -Uri "http://localhost:3000/v1/auth/login" -Method Post -ContentType "application/json" -Body '{"username":"admin","password":"REDACTED"}').token
...
| `JWT_SECRET` | `REDACTED` | JWT signing secret |
| `JWT_EXPIRY_HOURS` | `24` | Token expiration time |
| `ADMIN_USERNAME` | `admin` | Admin login username |
| `ADMIN_PASSWORD` | `REDACTED` | Admin login password |
